...
test("fully cited local facts can be review-ready without creating a proposal", () => {
  ...
  assert.deepEqual(summarizeLocalProposalQuality([audit]).totals, { audited: 1, reviewReady: 1, blocked: 0, notActionable: 0, addedEvidence: 1 });
...
test("harness import closure is local and rejects prohibited runtime dependencies", () => {
  ...
  const imports = [...source.matchAll(/^\s*import(?:[\s\S]*?\s+from\s+)?\s*["']([^"']+)["']/gm)].map((match) => match[1]);
