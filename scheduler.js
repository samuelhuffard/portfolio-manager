...
function wrapJob(name, label, fn) {
  return async (...args) => {
...
cron.schedule("30 9 * * 1-5", wrapJob("holdings-sync", "Holdings", syncHoldings), TZ);

// ── Holdings sync (4:30 PM ET — after close) ────────────────────────────────
cron.schedule("30 16 * * 1-5", wrapJob("holdings-sync", "Holdings", syncHoldings), TZ);

// ── Pre-market (8:30 AM ET) ──────────────────────────────────────────────────
// Macro snapshot refresh, regime check, overnight news on held positions.
cron.schedule("30 8 * * 1-5", wrapJob("premarket-check", "Premarket", runPremarketCheck), TZ);

// ── Opening check (9:35 AM ET) ───────────────────────────────────────────────
// First look at opening prices — gap analysis, any alerts triggered at open.
cron.schedule("35 9 * * 1-5", wrapJob("intraday-monitor", "Opening", () => runIntradayMonitor({ context: "opening" })), TZ);
...
for (const time of ["0 11 * * 1-5", "0 13 * * 1-5", "0 15 * * 1-5"]) {
  cron.schedule(time, wrapJob("holdings-sync", "Holdings", syncHoldings), TZ);
}
...
cron.schedule("50 15 * * 1-5", wrapJob("intraday-monitor", "PreClose", () => runIntradayMonitor({ context: "pre-close" })), TZ);

// ── Exit monitor (4:45 PM ET) ────────────────────────────────────────────────
// Full ATR/fundamental/momentum exit signals with complete EOD bar data.
cron.schedule("45 16 * * 1-5", wrapJob("exit-monitor", "ExitMonitor", runExitMonitor), TZ);
...
cron.schedule("15 17 * * 1-5", wrapJob("research-scan", "Research", runResearchScan), TZ);

// ── Performance review (5:45 PM ET) ─────────────────────────────────────────
// Scores past recommendations whose 30/90/180-day windows have elapsed.
cron.schedule("45 17 * * 1-5", wrapJob("performance-review", "Performance", runPerformanceReview), TZ);
