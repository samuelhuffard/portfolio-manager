...
**Historical fix requested:** Same fix as F-2026-092 — teach the cron-check the actual Sun-Thu schedule so Fridays don't page as missed runs.

**Severity suggestion:** P3 (analyst; deterministic severity P1 stands until Sam edits it)

## Resolution

Fixed with F-2026-092 in the gate-closing release. Friday no longer expects the
Mon–Thu research-scan cadence, and the exact 2026-07-10 false-positive topology
is regression-tested in `tests/sysloop.test.js`. The full backend suite passed
713/713 before deployment.
