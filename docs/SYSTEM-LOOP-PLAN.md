...
|---|---|---|---|
| 1 | Jetson PM2 health | On Jetson: `pm2 jlist` → `portfolio-manager` online, with readable restart metadata and a prior baseline. `npm run deploy:restart` may explain a delta only through a contiguous chain of dedicated-key signed, exact `N → N+1` markers that bind host, process, branch, commit, start time, stable restart count, and zero exit code. The sentinel atomically consumes accepted markers while advancing its baseline. | Offline, missing metadata/baseline, counter reset, unstable/non-zero exit, or any unsigned/forged/stale/incomplete/replayed restart edge = P1 |
