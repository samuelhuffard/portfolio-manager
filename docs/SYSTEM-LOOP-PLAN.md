...
|---|---|---|---|
| 1 | Jetson PM2 health | On Jetson: `pm2 jlist` → `portfolio-manager` online, with readable restart metadata and a prior baseline. Until a trusted deploy marker exists, every positive restart-count delta is unexplained and blocking even when exit code is zero. | Offline, missing metadata/baseline, or any positive restart delta = P1 |
