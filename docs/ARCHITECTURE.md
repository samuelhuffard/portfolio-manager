...
1. **Universe** = agent watchlist (`config/agents/<id>/watchlist.json`) + up to 5 Robinhood scan candidates (`lib/market-scan-sync.js` pulls scans directly via robin_stocks before any agent runs; rows ranked by score, then |changePct|).
2. **Fundamentals** via `lib/yahoo.js` (yahoo-finance2 v3 — class API, `new YahooFinance()`), plus `lib/edgar.js` (SEC filings, keyless), `lib/fred.js` (macro, optional key), `lib/tavily.js` (news, shared per-ticker cache).
