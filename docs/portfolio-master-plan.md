...
| Restart investigation | TRUST | **Resolved for the observed count.** The first 35 PM2 restarts correlate to controlled `SIGINT` deployments; four additional gate-closing code/docs/cost-policy/logging restarts bring the current count to 39. PM2 remains `unstable_restarts=0`, `exit_code=0`, and online. Raw log timestamps are enabled and the saved PM2 process list preserves that setting. | Current live PM2/process evidence. The July 14 day is already ineligible; any future unexplained restart remains a gate failure. |
...
| G0.6 Explain infrastructure restarts — **verified complete for current evidence** | TRUST | ops investigator | All 39 observed restarts are accounted for: the original 35 controlled `SIGINT` deployments plus four controlled gate-closing code/evidence/cost-policy/logging restarts. PM2 reports online, `unstable_restarts=0`, and `exit_code=0`; `--time` is enabled and persisted with `pm2 save`. | Completed for the current count. Any future unexplained restart during the window invalidates the day. |
...
| G0.8 Gate-closing release verification — **backend verified; companion/default branch open** | BOTH | primary + independent reviewer | Backend `mandate-v3` is deployed and clean through current evidence head, with secret scan, 713/713 tests, migration 0007, controlled restart, health 200, fresh timestamped startup logs, transactional parity `MATCH`, non-persisting observer dry run, application restore, and unchanged shadow authority. Dashboard `bbb5a5c` passed 109/109, TypeScript, and build. | The local companion must still be explicitly approved/restarted to load `bbb5a5c`, and backend `main` alignment awaits explicit approval. Neither open item permits retroactive Day 1. |
