...
| Restart investigation | TRUST | **Resolved for the observed count.** The first 35 PM2 restarts correlate to controlled `SIGINT` deployments; three additional gate-closing code/docs/cost-policy restarts bring the current count to 38. PM2 remains `unstable_restarts=0`, `exit_code=0`, and online. | Current live PM2/process evidence. The July 14 day is already ineligible; any future unexplained restart remains a gate failure. |
...
| G0.6 Explain infrastructure restarts — **verified complete for current evidence** | TRUST | ops investigator | All 38 observed restarts are accounted for: the original 35 controlled `SIGINT` deployments plus three controlled gate-closing code/evidence/cost-policy restarts. PM2 reports online, `unstable_restarts=0`, and `exit_code=0`. | Completed for the current count. Any future unexplained restart during the window invalidates the day. |
