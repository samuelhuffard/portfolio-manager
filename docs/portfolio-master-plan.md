...
| Live release | BOTH | Production is clean on branch `mandate-v3` at backend commit `e162637`. | Current live verification. Parity, observer, and cost-governance changes still in the worktree are local/uncommitted and are not part of this live revision. |
...
| Remaining gate-closing work | BOTH | Split parity, the two-clock daily observer, and cost enforcement are implemented locally and under final review. The restore format has been repaired for exact numeric preservation and must be re-proved against the final migration set. Final review/release verification and the human-set monthly ceiling/credential isolation remain open. | Local code is not runtime proof; see HUMAN NEEDED and the gate rows below. |
...
| G0.2 Split parity semantics | TRUST | builder + money-path reviewer | Exact transactional comparison for ticker, shares, cost basis/average cost, cash, units, ownership/lots; valuation report contains quote value, source, and timestamp and only exact-compares shared snapshots | Tests plus production read-only proof. Any transactional divergence blocks/resets safety; stale valuation is a visible degradation, not a false accounting mismatch. |
...
| G0.7 Backup/restore drill — **re-proof pending final migration** | TRUST | ops owner + reviewer | The first drill exposed a JavaScript numeric-precision flaw during independent review. The logical format is repaired and regression-tested, but exact live proof must be rerun after the final migration set is deployed. | Do not count the superseded first manifest as gate proof. Provider-native Neon PITR remains a separate pre-canonical-cutover gate. |
