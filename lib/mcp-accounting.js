...
import { shadowWriteLot } from "./pg/dual-write.js";
import { consumeLotsFIFO, openLot } from "./tax-lots.js";
import { consumeOwnedLotsFIFO } from "./owned-lots.js";
import { LOT_UNATTRIBUTED } from "../contracts/lot.js";
import { assertApprovedProposalSignature } from "./proposal-signature.js";
import { checkSellOwnerShareLimitForExecution } from "../contracts/proposal.js";
...
export function validateMcpFillInput({ proposal, existingTrades = [], orderId, ticker, side, shares, price, agentId, signatureSecret }) {
  ...
  const sellScope = checkSellOwnerShareLimitForExecution(proposal);
  if (!sellScope.ok) throw new Error(`Proposal ${proposal.id} is not executable: ${sellScope.reason}.`);
  if (
    normalizedSide === "SELL" &&
    !sellScope.legacy &&
    parsedShares > proposal.sellOwnerShareLimit + 1e-8
  ) {
    throw new Error(
      `SELL fill ${parsedShares} shares exceeds the signed strategy-owner ceiling of ${proposal.sellOwnerShareLimit} shares.`
    );
  }
...
export function applyFillToLots(trade, lots, { enforceOwnership = false } = {}) {
  ...
  if (owned) {
    try {
      const { realizedGain, updatedLots } = consumeOwnedLotsFIFO(lots, {
      ...
      // Ownership-scoped SELL can't reconcile from the named strategy's lots. The
...
export async function recordMcpFill({ sheets, spreadsheetId, sheetIds, proposal, orderId, ticker, side, shares, price, agentId }) {
  ...
  // Sheets is authoritative. Shadow only after the lot writes succeed; each
  // call is flag-gated and failure-isolated inside the Postgres shadow layer.
  for (const lot of [...newLots, ...updatedLots]) await shadowWriteLot(lot);
  ...
  if (needsReconciliation) {
    ...
    await recordReconciliationNeeded({
      ...
      side: trade.side, shares: trade.shares, reason: reconciliationReason ?? "ownership-scoped SELL could not consume strategy-owned lots",
    });
  }
