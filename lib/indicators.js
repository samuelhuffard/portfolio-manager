...
/**
 * Relative strength of a name vs its sub-vertical benchmark. Compares the ratio
 * (nameClose / benchClose) week over week and reports how many of the last `weeks`
 * weeks the ratio fell — Trigger 2 fires on 3+ consecutive declining weeks. Returns
 * null if the two series can't be aligned with enough history.
 */
...
/** Map a Yahoo sector/industry to Agent One v5's sub-vertical, or null if out of universe. */
export function classifySubVertical({ sector, industry } = {}) {
  ...
  if (
    ...
    (ind.includes("communication equipment") ||
      ind.includes("computer hardware") ||
      ind.includes("electronic components") ||
      ind.includes("scientific") ||
      ind.includes("network") ||
      ind.includes("storage"))
  ) {
  ...
  if (
    ...
    ind.includes("health information") ||
    ind.includes("healthcare information") ||
    ind.includes("medical information") ||
    ind.includes("defense") ||
    ind.includes("aerospace") ||
    ind.includes("internet retail") ||
    ind.includes("internet content")
  ) {
    return "Tech-Adjacent High-Growth";
  }
