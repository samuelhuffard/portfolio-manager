...
const EXCLUDED_NAME_PATTERN =
  /\b(warrant|right|rights|unit|units|preferred|preference|depositary|depository|ADS|ADR|ETN|exchange traded|fund|trust|notes? due|%)/i;
 ...
 */
export function selectEnrichmentBatch(catalog, { perRun = 250, staleDays = 30, now = new Date() } = {}) {
  ...
  return [...never, ...stale].slice(0, perRun).map((e) => e.t);
}

/**
 * Adapt catalog entries into the shape lib/screener.js screenUniverse expects
 * ({ ticker, subVertical, marketCap, avgDollarVolume }), skipping entries whose
 * sector hasn't been enriched yet — screenUniverse would reject them anyway,
 * and this keeps its rejection log for real out-of-universe names only.
 */
