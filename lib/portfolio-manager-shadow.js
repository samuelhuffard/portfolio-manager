...
/**
 * Pure, deterministic Kairos (Agent 4) shadow evaluation. All state and time are explicit
...
export function evaluateShadowPortfolioDecision(rawRequest) {
  ...
  if (request.originatorId === PORTFOLIO_MANAGER_ID) {
    reject("REJECT_MANAGER_ORIGINATED", "Kairos cannot originate a specialist proposal.");
  ...
  if (canonicalJson(proposal) !== canonicalJson(request.reviewedProposal)) {
    reject("REJECT_PROPOSAL_MUTATED", "Kairos must accept or reject the specialist proposal without changing it.");
  }
