...
// lots cannot top up a named strategy's SELL. Unattributed (legacy/manual)
// SELLs fall back to the account-wide path, since no strategy owns those lots.
// An attributed SELL that exceeds its own owned shares warns (batch survives)
// and routes to reconciliation rather than consuming quarantined inventory.
    ...
    if (fill.side === "BUY") {
      ...
      try {
        const { realizedGain: gain, updatedLots } = owned
          ? consumeOwnedLotsFIFO(workingLots, { ticker: fill.ticker, agentId, sharesToSell: fill.shares, sellPricePerShare: fill.price })
