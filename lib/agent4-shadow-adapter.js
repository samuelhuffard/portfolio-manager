...
/**
 * Builds a Kairos (Agent 4) review request from the proposal just persisted by the
...
export function buildAgent4ShadowReviewRequest({ proposal, recommendation, context, policy, now = new Date() }) {
  ...
  if (!(equity > 0)) throw new Error("Kairos shadow review requires a positive portfolio equity snapshot.");
  ...
  return {
    ...
    allocationSnapshot: {
      ...
      budgets: SPECIALIST_IDS.map((agentId) => ({
        ...
        // Kairos begins with no claimed performance evidence. Later score and
...
/** Persist a Kairos shadow observation when the reviewed policy is active. */
