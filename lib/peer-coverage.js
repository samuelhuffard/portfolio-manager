 ...
 */
export function selectPeerCoverageRefreshTargets(catalog = {}, requests = {}, peerMetrics = {}, { limit = Infinity } = {}) {
  const unresolved = {};
  for (const [ticker, request] of Object.entries(requests ?? {})) {
    ...
    if (!coverage.ready) unresolved[ticker] = request;
  }
  const targets = coveragePriorityTickers(catalog, unresolved)
    .filter((ticker) => !isDataComplete(peerMetrics?.[ticker], COVERAGE_CORE_METRICS))
