...
import { getRedis, listAllProposals, getBreakerState, getPortfolioHighWaterMark } from "../redis.js";
...
// Known gap (docs/SYSTEM-LOOP-PLAN.md, RISK_REGISTER): no market-holiday
// awareness anywhere in the system yet. Weekdays only.
...
async function gatherRedisSlices() {
  ...
  if (!redis) return { state: null, proposals: null, listIds: null, breaker: null, hwm: null, companionMs: null, jobRuns: null };
  const [state, proposals, listIds, breaker, hwm, companionRaw, jobRunsRaw] = await Promise.all([
  ...
  return { state: parsedState, proposals, listIds, breaker, hwm, companionMs, jobRuns: Array.isArray(jobRunsRaw) ? jobRuns : null };
}
  ...
  try {
    ...
    for (const doc of fs.readdirSync(docsDir).filter((f) => f.endsWith(".md"))) {
      ...
      for (const m of text.matchAll(/\b((?:lib|jobs|scripts|config|tests)\/[A-Za-z0-9_./-]+\.(?:js|mjs|py|json|md))\b/g)) {
        ...
        // dashboard repo — check the sibling repo before calling it drift.
        const existsHere = fs.existsSync(path.join(repoRoot, rel));
        const existsInDashboard = fs.existsSync(path.join(repoRoot, "..", "portfolio-dashboard", rel));
