...
import { openFindingsSummary } from "./findings.js";
import {
  PM2_DEPLOY_MARKER_MAX_CHAIN,
  getSysloopDeployHmacSecret,
  pm2DeployMarkerKey,
  validatePm2DeployMarkerChain,
} from "./deploy-marker.js";
  ...
  try {
    ...
    return list.map((p) => ({
      ...
      pm2_env: {
        ...
        SYSLOOP_DEPLOYED_BRANCH: p.pm2_env?.SYSLOOP_DEPLOYED_BRANCH,
        SYSLOOP_DEPLOYED_COMMIT: p.pm2_env?.SYSLOOP_DEPLOYED_COMMIT,
      },
    }));
  } catch (e) {
...
async function gatherRedisSlices() {
  ...
  if (!redis) return { state: null, proposals: null, listIds: null, breaker: null, hwm: null, companionMs: null, jobRuns: null, openReconciliations: null, phase0ObservationDates: null };
  const [state, proposals, listIds, breaker, hwm, companionRaw, jobRunsRaw, openReconciliations, phase0ObservationDates] = await Promise.all([
    ...
    redis.lrange("pm:phase0-observation:index", 0, 89).catch(() => null),
  ]);
  ...
  return {
    ...
    hwm,
    companionMs,
    jobRuns: Array.isArray(jobRunsRaw) ? jobRuns : null,
    openReconciliations,
    phase0ObservationDates: Array.isArray(phase0ObservationDates) ? phase0ObservationDates.filter((date) => typeof date === "string") : null,
  };
}

async function gatherPendingDeployMarkers(processName, previousRestarts, currentRestarts) {
  if (!Number.isInteger(previousRestarts) || !Number.isInteger(currentRestarts) || currentRestarts <= previousRestarts) return [];
  if (currentRestarts - previousRestarts > PM2_DEPLOY_MARKER_MAX_CHAIN) return null;
  const redis = getRedis();
  ...
  const rows = await redis.mget(...keys).catch(() => null);
  if (!Array.isArray(rows) || rows.length !== keys.length) return null;
  return rows.map((raw) => {
...
export async function assembleSnapshot({ repoRoot, now = new Date() } = {}) {
  ...
  const processName = process.env.SYSLOOP_PM2_NAME?.trim() || "portfolio-manager";
  const target = process.env.SYSLOOP_DEPLOY_TARGET?.trim() || "jetson";
  const currentProcess = pm2?.find((entry) => entry.name === processName);
  const restartDelta = Number.isInteger(currentProcess?.pm2_env?.restart_time) && Number.isInteger(prevState.restarts)
    ? currentProcess.pm2_env.restart_time - prevState.restarts
    : null;
  const pendingDeployMarkers = restartDelta > 0
    ? await gatherPendingDeployMarkers(processName, prevState.restarts, currentProcess.pm2_env.restart_time)
    : [];
  let markerSecret = null;
  try { markerSecret = getSysloopDeployHmacSecret(); } catch { markerSecret = null; }
  const deployMarkerTrust = restartDelta > 0
    ? validatePm2DeployMarkerChain(pendingDeployMarkers, {
      secret: markerSecret,
      target,
      host: os.hostname(),
      processName,
      previousRestarts: prevState.restarts,
      currentRestarts: currentProcess?.pm2_env?.restart_time,
      previousUnstableRestarts: prevState.unstableRestarts,
      currentUnstableRestarts: currentProcess?.pm2_env?.unstable_restarts,
      currentExitCode: currentProcess?.pm2_env?.exit_code,
      currentStartedAt: currentProcess?.pm2_env?.pm_uptime,
      currentBranch: currentProcess?.pm2_env?.SYSLOOP_DEPLOYED_BRANCH,
      currentCommit: currentProcess?.pm2_env?.SYSLOOP_DEPLOYED_COMMIT,
      nowMs: now.getTime(),
    })
    : { trusted: null, reason: "not_required", markerIds: [], markerKeys: [] };

  const inputs = {
    pm2: {
      processes: pm2,
      prevRestarts: prevState.restarts,
      prevUnstableRestarts: prevState.unstableRestarts,
      deployMarkerTrust,
    },
    health: { health },
    nowMs: now.getTime(),
    ...
    phase0: {
      observedDates: redisSlices.phase0ObservationDates,
      expectedPriorDate: lastTradingDayISO(new Date(now.getTime() - 24 * 60 * 60 * 1000)),
    },
    dashboard: { probes },
    ...
    trackedFindings: { open: openFindingsSummary(path.join(repoRoot, "ops", "findings")) },
  };
  ...
  const snapshot = {
    ...
    stats: {
      ...
      pm2DeployMarker: {
        trusted: deployMarkerTrust.trusted,
        reason: deployMarkerTrust.reason,
        markerIds: deployMarkerTrust.markerIds,
      },
    },
  ...
  const nextState = {
    restarts: currentProcess?.pm2_env?.restart_time ?? prevState.restarts,
    unstableRestarts: currentProcess?.pm2_env?.unstable_restarts ?? prevState.unstableRestarts,
    ...
    lastDeployMarkerIds: deployMarkerTrust.trusted ? deployMarkerTrust.markerIds : [],
  };

  return {
    snapshot,
    nextState,
    prevAnomalyFingerprints: prevState.prevAnomalyFingerprints ?? [],
    consumedDeployMarkerKeys: deployMarkerTrust.trusted ? deployMarkerTrust.markerKeys : [],
  };
}
...
export async function persistSnapshot({
  ...
  const keys = [SNAPSHOT_KEY(snapshot.date), STATE_KEY, LAST_RUN_KEY, ...consumedDeployMarkerKeys];
  if (keys.some((key) => !key.startsWith("pm:sysloop:"))) {
    throw new Error("sysloop attempted atomic persistence outside its namespace.");
  }
  await redis.eval(PERSIST_SNAPSHOT_SCRIPT, keys, [
    JSON.stringify(snapshot),
    JSON.stringify(nextState),
    JSON.stringify({ ts: snapshot.ts, date: snapshot.date, anomalyCount: snapshot.anomalies.length }),
    String(SNAPSHOT_TTL),
  ]);
  return true;
}
