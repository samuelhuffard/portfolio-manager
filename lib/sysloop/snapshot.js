...
export async function assembleSnapshot({ repoRoot, now = new Date() } = {}) {
  ...
  const inputs = {
    ...
    // `?? []` so a global Redis outage (already reported by health/redisQueue
    // checks) doesn't also fire a duplicate reconciliation P1; when Redis is up,
    // any durable unreconciled-SELL record surfaces here every run.
