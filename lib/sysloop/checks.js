...
// ── 1. Jetson PM2 health ─────────────────────────────────────────────────────
export function checkPm2({ processes, prevRestarts, prevUnstableRestarts, processName = "portfolio-manager" } = {}) {
  ...
  if (!Number.isFinite(restarts)) {
    ...
    if (unstableDelta > 0 || (Number.isFinite(exitCode) && exitCode !== 0)) {
      ...
      out.push(anomaly(
        ...
        `${restarts - prevRestarts} restart(s) occurred without a trusted deploy marker; unstable delta=${unstableDelta ?? "unknown"}; last exit=${exitCode ?? "unknown"}`,
      ));
    }
  }
  return out.concat([]);
...
export function checkResearchDataHealth({ researchData, enabled = false, nowMs = Date.now() } = {}) {
  ...
  if (researchData.state === "failed") {
    return [anomaly("research-data", "P1", "Research-data refresh failed", `failureStage=${researchData.failureStage ?? "unknown"}`, `failed:${researchData.failureStage ?? "unknown"}`)];
  }
  ...
  if (researchData.state === "running") {
    ...
    if (nowMs - startedMs > RESEARCH_DATA_RUNNING_MAX_MS) return [anomaly("research-data", "P1", "Research-data refresh is stuck", `running for ${Math.round((nowMs - startedMs) / 60000)}m`, "running:stale")];
    return [];
  }
  ...
  if (counts.some((count) => !Number.isFinite(count) || count < 0) || cataloged === 0 || classified > cataloged || metricRows > cataloged) {
    return [anomaly("research-data", "P1", "Research-data refresh reported implausible counts", `cataloged=${researchData.cataloged}, classified=${researchData.classified}, metricRows=${researchData.metricRows}`, "counts:implausible")];
  }
  return [];
}
...
export const JOB_EXPECTATIONS = {
  ...
  "order-reconciliation": { byHourET: 17 },
  "intraday-monitor": { byHourET: 16.5 },
  "exit-monitor": { byHourET: 17.25, weekdays: ["Mon", "Tue", "Wed", "Thu"] },
  "research-scan": { byHourET: 18, weekdays: ["Mon", "Tue", "Wed", "Thu"] },
