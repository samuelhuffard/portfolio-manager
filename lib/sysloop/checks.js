import { fingerprint, isKnownYahooValidationNoise } from "./fingerprint.js";
...
export const JOB_EXPECTATIONS = {
  ...
  // A classified partial scan is a SKILL/P2 finding even though the process
  // throws; a total or unclassified failure remains the ordinary P2 failure.
  "research-scan": { byHourET: 18, weekdays: ["Mon", "Tue", "Wed", "Thu"], severity: "P2", outcomeFindings: new Set(["degraded", "not_configured"]) },
  "performance-review": { byHourET: 18.2, severity: "P2" },
  "verify-ledgers": { byHourET: 18.25, severity: "P1" },
  // The 20:10 final sentinel is the first same-day observer after this 19:30
  // advisory workflow. Its factual non-configured/degraded receipt must be
  // visible even when the process itself completed (`ok: true`).
  "research-data-refresh": { byHourET: 20, severity: "P2", outcomeFindings: new Set(["not_configured", "degraded"]) },
};
...
export function checkJobFreshness({ lastRuns, nowET, isTradingDay } = {}) {
  ...
  for (const [job, exp] of Object.entries(JOB_EXPECTATIONS)) {
    ...
    } else if (exp.outcomeFindings?.has(run.outcome)) {
      const reason = typeof run.outcomeReason === "string" ? run.outcomeReason : "unknown";
      out.push(anomaly(
        "cron",
        exp.severity,
        `Job ${job} completed with outcome ${run.outcome}`,
        `outcomeReason=${reason}`,
        `${job}:outcome:${run.outcome}:${reason}`,
      ));
...
// A scheduler or host outage can prevent the sentinel itself from running on a
// trading day. The next sentinel must therefore look backward and make the
// missing immutable Phase 0 record visible, rather than treating today's fresh
// process as proof that the missed day was observed.
export function checkPhase0ObservationContinuity({ observedDates, expectedPriorDate } = {}) {
  if (!Array.isArray(observedDates)) {
    // This is backward-looking observability, not evidence that today's money
    // path is unsafe. A P1 here would make a host outage fail both its own
    // safety day and the next day's TRUST gate through open_p0_p1.
    return [unavailable("phase0-continuity", "Phase 0 observation index is unreadable", "P2")];
  }
  if (!expectedPriorDate) return [];
  if (observedDates.includes(expectedPriorDate)) return [];
  return [anomaly(
    "phase0-continuity",
    "P2",
    `Phase 0 observation for ${expectedPriorDate} is missing`,
    "The prior trading day has no immutable observation record; a scheduler or host outage may have left safety evidence unreviewed.",
    `missing:${expectedPriorDate}`,
  )];
}

// A monitor may conservatively degrade individual names when source evidence is
// unavailable, but an all-degraded run leaves no position genuinely monitored.
// Keep the raw coverage in the receipt and let the sentinel own this severity.
export function checkExitMonitorCoverage({ lastRuns, nowET, isTradingDay } = {}) {
  if (!isTradingDay || !["Mon", "Tue", "Wed", "Thu"].includes(nowET?.weekday)) return [];
  const nowHour = nowET.hour + nowET.minute / 60;
  if (nowHour < JOB_EXPECTATIONS["exit-monitor"].byHourET) return [];
  const run = lastRuns?.["exit-monitor"];
  if (!run || run.dateET !== nowET.date) return [];
  const coverage = run.evidence?.holdingMonitoring;
  if (!holdingMonitorCoveragePasses(coverage)) {
    return [anomaly(
      "holding-monitoring",
      "P1",
      "Exit monitor reported invalid holding coverage",
      "The exit-monitor receipt has malformed, failed, overflow, or silently skipped holding coverage.",
      "exit-monitor:invalid-coverage",
    )];
  }
  if (coverage.expected > 0 && coverage.monitored === 0) {
    return [anomaly(
      "holding-monitoring",
      "P1",
      "Exit monitor degraded every held position",
      `expected=${coverage.expected}, monitored=0, degraded=${coverage.degraded}; no held position received usable exit monitoring.`,
      "exit-monitor:all-degraded",
    )];
  }
  if (coverage.degraded > 0) {
    return [anomaly(
      "holding-monitoring",
      "P2",
      "Exit monitor has partially degraded holding coverage",
      `expected=${coverage.expected}, monitored=${coverage.monitored}, degraded=${coverage.degraded}.`,
      "exit-monitor:partial-degraded",
    )];
  }
  return [];
}
...
// five-trading-day window, >=3 genuine actionable proposals reached the approval
...
// window is maturing (default >=4 trading days elapsed), so a legitimately quiet
...
export function checkLogClusters({ clusters, prevClusters } = {}) {
  ...
  for (const c of clusters) {
    ...
    const knownYahooNoise = isKnownYahooValidationNoise(c.exemplar);
    if (before === undefined) {
      if (c.count >= NEW_CLUSTER_MIN) {
        out.push(anomaly(
          "logs",
          "P2",
          knownYahooNoise ? "Known Yahoo provider-data degradation" : "New error cluster in PM2 logs",
          `${c.count}× "${c.exemplar.slice(0, 160)}"${knownYahooNoise ? " — research remains fail-closed" : ""}`,
          `cluster:${c.fingerprint}`,
        ));
      }
    } else if (c.count >= CLUSTER_GROWTH_MIN && c.count >= before * CLUSTER_GROWTH_FACTOR) {
      out.push(anomaly(
        "logs",
        knownYahooNoise ? "P2" : "P1",
        knownYahooNoise ? "Known Yahoo provider-data degradation" : "Error cluster growing fast",
        `${before}→${c.count}× "${c.exemplar.slice(0, 160)}"${knownYahooNoise ? " — research remains fail-closed" : ""}`,
        `cluster-growth:${c.fingerprint}`,
      ));
    }
  }
  return out;
...
export function runChecks(inputs) {
  return [
    ...
    ...checkExitMonitorCoverage(inputs.jobs),
    ...checkPhase0ObservationContinuity(inputs.phase0),
