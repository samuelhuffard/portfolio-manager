...
// Yahoo quoteSummary schema drift is external research-data degradation. It
// remains observable and fails affected research candidates closed, but it is
// not a broker, ledger, execution, or proposal-control incident.
const YAHOO_VALIDATION_NOISE_RE = /(?:yahoo-finance2 version out of date|did not validate with schema|failed yahoo schema validation|netSharePurchaseActivity\.(?:netInstSharesBuying|netInstBuyingPercent)|information on how to turn off the above logging)/i;

export function isKnownYahooValidationNoise(line) {
  return YAHOO_VALIDATION_NOISE_RE.test(String(line));
}
