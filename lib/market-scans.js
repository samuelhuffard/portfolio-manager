...
export function normalizeMarketScanRows(input) {
  ...
  return rows
    ...
    .sort((a, b) => {
      ...
      // Robinhood-sourced rows all arrive score-less, so the tiebreak decides
      // what agents actually research. Alphabetical meant "the A-names" every
      // day — rank by move magnitude instead so the strongest signals surface.
      const moveA = Math.abs(a.changePct ?? 0);
      const moveB = Math.abs(b.changePct ?? 0);
