...
export function validateActionableEvidence({ action, thesis, evidenceCitations, evidenceIds, claimedBusinessFamily = null }) {
  if (!["BUY", "SELL"].includes(action)) return { valid: true, issues: [] };
  const sentences = sentenceList(thesis);
  const citations = Array.isArray(evidenceCitations) ? evidenceCitations : [];
  const issues = [];
  if (!sentences.length) issues.push("actionable thesis is empty");
  ...
  for (const sentence of sentences) {
    const citation = citations.find((item) => item?.claim?.trim() === sentence);
    if (!citation) {
      issues.push(`thesis sentence has no exact evidence citation: ${sentence.slice(0, 120)}`);
      continue;
    }
    const ids = Array.isArray(citation.evidence_ids) ? citation.evidence_ids : [];
    if (!ids.length || ids.some((id) => typeof id !== "string" || !evidenceIds.has(id))) {
      issues.push(`thesis sentence cites missing or unknown evidence: ${sentence.slice(0, 120)}`);
    }
  }
...
- The EVIDENCE LEDGER below is the complete set of facts you may use. Each item has an ID and a type. Cite the exact ID(s) for EVERY sentence in an actionable BUY/SELL thesis using evidence_citations.
...
- Do not use UNVERIFIED claims as support for BUY/SELL. If a thesis cannot be fully cited, return HOLD.
Respond with ONLY a single JSON object — no markdown fences, no commentary before or after — matching exactly this shape:
{
  ...
  "evidence_citations": [{"claim": "<an exact full sentence from thesis>", "evidence_ids": ["<one or more EVIDENCE LEDGER IDs>"]}],
...
${newsBlock}`;
  ...
  const createRequest = (formatRecovery = false) => ({
  ...
  let recommendation = parseRecommendation(initialText, ticker, evidenceIds);
  let response = initialResponse;
  let formatRecoveryAttempted = false;

  // A malformed response is an operational failure, not an investment HOLD. One
  // bounded retry is worthwhile because this is normally a cut-off JSON object,
  // not a second research opinion or a change in the evidence/decision policy.
  if (recommendation.outputInvalid) {
    formatRecoveryAttempted = true;
    console.warn(`[AI Overlay] ${ticker}: ${initialResponse.stop_reason === "max_tokens" ? "response hit max_tokens" : "response was invalid"}; retrying once for a compact structured response.`);
    response = await invoke(createRequest(true), "generator_format_retry");
    const recoveryText = response.content.find((b) => b.type === "text")?.text ?? "";
    recommendation = parseRecommendation(recoveryText, ticker, evidenceIds);
  }
...
export function parseRecommendation(text, ticker, evidenceIds = null) {
  ...
  try {
    ...
    const evidenceCheck = evidenceIds
      ? validateActionableEvidence({ action, thesis: parsed.thesis, evidenceCitations, evidenceIds, claimedBusinessFamily })
      : { valid: true, issues: [] };
