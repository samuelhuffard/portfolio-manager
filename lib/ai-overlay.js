...
${newsBlock}`;
  ...
  if (response.stop_reason === "max_tokens") {
    console.warn(`[AI Overlay] ${ticker}: response hit max_tokens — invalid JSON is recorded as a review error, never an investment HOLD.`);
  }
  const text = response.content.find((b) => b.type === "text")?.text ?? "";
  return { ...parseRecommendation(text, ticker, evidenceIds), evidencePacket, modelStopReason: response.stop_reason };
}
