...
async function athenaGet(path, { fetchImpl = fetch, env = process.env, timeoutMs = ATHENA_TIMEOUT_MS } = {}) {
  ...
  try {
    ...
    console.error(`[Athena] GET ${path} failed (continuing without): ${err.message}`);
    return null;
  }
}
...
export async function fetchAthenaDossier(ticker, opts = {}) {
  return athenaGet(`/api/agent/ticker/${encodeURIComponent(ticker)}`, opts);
}
