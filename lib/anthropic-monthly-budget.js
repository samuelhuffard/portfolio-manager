...
export function createAnthropicMonthlyBudget({ env = process.env, now = () => new Date(), redis = getRedis() } = {}) {
  ...
  async function authorizeCall({ role, model, request = null, reserveUsd = null, protectedCapacity = false }) {
    ...
    if (cap.status === "NOT_CONFIGURED") {
      // Production sets ANTHROPIC_BUDGET_REQUIRED=true so a lost/typoed ceiling
      // env var refuses calls instead of silently removing the monthly cap.
      if (String(env.ANTHROPIC_BUDGET_REQUIRED ?? "").trim().toLowerCase() === "true") {
        throw new MonthlyAnthropicBudgetError(
          "ANTHROPIC_BUDGET_REQUIRED=true but ANTHROPIC_MONTHLY_MAX_USD is not configured; refusing an unbudgeted Anthropic call.",
          "monthly_budget_config_invalid"
        );
      }
  ...
  async function settleCall(authorization, telemetryResult) {
    ...
    if (record.usageComplete === false) {
      // The provider response carried no token counts, so the call cannot be
      // priced. Never settle it as a $0 success; charge the full reservation.
      console.error(`[AnthropicBudget] ${active.role} response for ${active.model} had no usage token counts; settling conservatively at its $${active.reserveUsd.toFixed(4)} reservation.`);
      await atomicSettle(active, "AMBIGUOUS_FAILURE", 0);
