...
/**
 * Agent Three cannot invest in a company without a multi-year record (owner decision
 * 2026-08-02). This is an ELIGIBILITY gate, not a scoring outcome: a young company is
 * never a valid Agent Three candidate, so it is excluded here rather than scored and
 * discarded downstream, and the rejection reason stays legible instead of surfacing as
 * an unexplained low score.
 *
 * Note the scorer applies a stricter *evidence* bar of its own — three year-over-year
 * comparisons across non-overlapping TTM windows need ~4 years of filings
 * (lib/agent3-history.js). This gate is the mandate rule; that one is what the data can
 * actually answer. A name passing here can still score partial.
 */
export const AGENT_THREE_MIN_PUBLIC_YEARS = 3;

/**
 * Listing date → years public. Accepts the epoch-millisecond number the catalog stores
 * (Yahoo `firstTradeDateMilliseconds`), a Date, or an ISO string, because the quote
 * provider has returned more than one of these shapes across versions. Anything else is
 * null — unverifiable, never guessed.
 */
function publicYears(candidate, now) {
  const raw = candidate?.firstTradeDate ?? candidate?.listingDate ?? candidate?.ipoDate ?? null;
  if (raw == null) return null;
  const listed = raw instanceof Date ? raw.getTime() : typeof raw === "number" ? raw : Date.parse(raw);
  if (!Number.isFinite(listed) || listed <= 0 || listed > now) return null;
  return (now - listed) / (365.25 * 86400000);
}

function screenAgentThree(candidates, limits) {
  const passed = [];
  const rejected = [];
  // Chronology is caller-supplied where it matters: a backtest or replay must be able to
  // ask "was this eligible THEN", not "is it eligible now".
  const now = Number.isFinite(Date.parse(limits.asOf)) ? Date.parse(limits.asOf) : Date.now();
  const minAvgDollarVolume = finiteNonnegative(limits.minAvgDollarVolume) ? limits.minAvgDollarVolume : 10_000_000;
  const minPublicYears = finiteNonnegative(limits.minPublicYears) ? limits.minPublicYears : AGENT_THREE_MIN_PUBLIC_YEARS;

  for (const candidate of candidates) {
    const yearsPublic = publicYears(candidate, now);
    if (yearsPublic == null) {
      reject(rejected, candidate, "public_history_unavailable", "listing date unavailable — Agent Three's multi-year record requirement is unverifiable");
      continue;
    }
    if (yearsPublic < minPublicYears) {
      reject(rejected, candidate, "public_history_below_floor", `public for ${yearsPublic.toFixed(1)}y, below Agent Three's ${minPublicYears}y minimum record`);
      continue;
    }
    if (!finiteNonnegative(candidate.avgDollarVolume)) {
