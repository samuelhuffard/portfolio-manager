...
import { assertOperationalLedgerEntries, assertPerformanceSourceInvocationEntries, assertPerformanceSourceRequestEntries, computePerformanceSourceInvocationHmac, computePerformanceSourceRequestHmac, signOperationalLedgerEntry } from "./operational-ledger.js";
import { parseLotRow, parsePerformanceRow, parseTradeRow, parseWithdrawalOperationRow } from "./operational-ledger-rows.js";
...
const TABS = ["Overview", "Holdings", "Market Scans", "Performance", "Trade Ledger", "Lots", "Investors", "Withdrawal Operations", "Track Record", ...AGENT_TAB_NAMES];
...
export async function ensureTabs(sheets, spreadsheetId) {
  ...
  // Seed row-1 headers for every newly created tab the schema check expects.
  // Writers call ensureHeadersExtendable lazily, which is fine for tabs a
  // scheduled job touches daily — but a tab whose only writer is a rare manual
  // operation (Withdrawal Operations) would otherwise sit header-less for
  // months and the sysloop schema check would report it as a standing P1
  // "missing or unreadable". Re-seeding is a no-op: ensureHeadersExtendable
  // returns early once the header row is at least as wide as expected.
  const seedable = missing.filter((tab) => SYSLOOP_EXPECTED_HEADERS[tab]?.row === 1);
  if (seedable.length) {
    const created = await sheets.spreadsheets.get({ spreadsheetId });
    const idByTitle = new Map(created.data.sheets.map((s) => [s.properties.title, s.properties.sheetId]));
    for (const tab of seedable) {
      await ensureHeadersExtendable(sheets, spreadsheetId, idByTitle.get(tab), tab, SYSLOOP_EXPECTED_HEADERS[tab].headers);
    }
  }
}
...
const PERFORMANCE_HEADERS = ["Date", "Portfolio Value", "S&P 500 (SPY)", "Units Outstanding", "NAV per Unit", "Row HMAC", "Source Request ID", "Source Request HMAC", "Source Invocation ID", "Source Invocation HMAC"];

export async function appendPerformanceRow(sheets, spreadsheetId, sheetId, { date, portfolioValue, spyPrice, unitsOutstanding, navPerUnit, sourceRequestId = null, sourceInvocationId = null }) {
  // A signed 16:30 invocation row is what prices real capital, so this writer
  // must not mint one from an invocation id alone. writePortfolioSnapshot
  // enforces the same pairing, but this is the exported function that actually
  // signs the row — keep the guard where the signature is produced.
  if (sourceInvocationId && !sourceRequestId) {
    throw new Error("A Performance source invocation ID requires its verified source request ID.");
  }
  ...
  const sourceInvocationHmac = computePerformanceSourceInvocationHmac({ ...entry, sourceInvocationId });
  await sheets.spreadsheets.values.append({
    spreadsheetId,
    range: "Performance!A1",
    valueInputOption: "RAW",
    insertDataOption: "INSERT_ROWS",
    requestBody: { values: [[date, portfolioValue, spyPrice ?? "", unitsOutstanding ?? "", navPerUnit ?? "", entry.rowHmac, sourceRequestId ?? "", sourceRequestHmac ?? "", sourceInvocationId ?? "", sourceInvocationHmac ?? ""]] },
  });
}

/** Full performance history, oldest first by accounting date (excludes header row). */
export async function readPerformanceHistory(sheets, spreadsheetId, { verify = true } = {}) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Performance!A2:J", valueRenderOption: "UNFORMATTED_VALUE" });
  const entries = (res.data.values || []).filter((row) => row[0]).map(parsePerformanceRow);
  const verifiedEntries = verify
    ? assertPerformanceSourceInvocationEntries(assertPerformanceSourceRequestEntries(assertOperationalLedgerEntries("performance", entries)))
    : entries;
  return verifiedEntries.sort((left, right) => String(left.date).localeCompare(String(right.date)));
}
...
export async function applyLotUpdatesToSheet(sheets, spreadsheetId, updatedLots) {
  ...
  const readIdentities = async () => {
    const response = await sheets.spreadsheets.values.batchGet({ spreadsheetId, ranges: identityRanges, valueRenderOption: "UNFORMATTED_VALUE" });
    return response.data.valueRanges ?? [];
  };
  const identitiesMatch = (ranges) => ranges.length === updatedLots.length
    && ranges.every((response, index) => String(response.values?.[0]?.[0] ?? "") === String(updatedLots[index].lotId));

  if (!identitiesMatch(await readIdentities())) {
  ...
  // The Sheets API has no compare-and-set, so a UI sort landing between the
  // check above and the write leaves a window no application lock can close.
  // Re-read the same identity cells afterwards and stop if they moved. This is
  // deliberately conservative: it also fires when the write was correct and the
  // sort happened just after, because the two are indistinguishable from here.
  //
  // This is DETECTION, not repair. If the write did land on the wrong rows it
  // wrote one lot's shares/status/HMAC onto another lot, and that row will now
  // fail `assertOperationalLedgerEntries("lot", …)` — which every verified
  // readAllLots performs, including the one a same-key retry does first. So a
  // retry cannot reconcile this; it will refuse on the corrupted row. Say that
  // plainly rather than sending the operator into a retry loop.
  if (!identitiesMatch(await readIdentities())) {
    throw new Error(
      `Lot rows moved during the update — rows ${updatedLots.map((lot) => lot.rowIndex + 2).join(", ")} no longer hold lots ${updatedLots.map((lot) => lot.lotId).join(", ")}. `
      + `The write may have landed on the wrong rows. A retry CANNOT repair this: verify the Lots tab by hand against the signed plan in Withdrawal Operations (run npm run ledgers:verify to find rows failing signature) before re-running anything.`
    );
  }
}

// ── Withdrawal Operations (signed, append-only recovery plans) ────────────

const WITHDRAWAL_OPERATION_HEADERS = ["Operation ID", "Created At", "Plan JSON", "Row HMAC"];

/**
 * Persist the exact lot allocation and signed investor entry before touching
 * Trade Ledger, Lots, or Investors. This is deliberately a separate,
 * append-only receipt: retries must replay this immutable plan rather than
 * recomputing FIFO against lots that may already have been consumed.
 */
export async function appendWithdrawalOperation(sheets, spreadsheetId, sheetId, operation) {
  await ensureHeadersExtendable(sheets, spreadsheetId, sheetId, "Withdrawal Operations", WITHDRAWAL_OPERATION_HEADERS);
  const signed = signOperationalLedgerEntry("withdrawal_operation", operation);
  await sheets.spreadsheets.values.append({
    spreadsheetId,
    range: "Withdrawal Operations!A1",
    valueInputOption: "RAW",
    insertDataOption: "INSERT_ROWS",
    requestBody: { values: [[signed.operationId, signed.createdAt, signed.planJson, signed.rowHmac]] },
  });
}

/** Read and verify every immutable withdrawal recovery plan. */
export async function readWithdrawalOperations(sheets, spreadsheetId, { verify = true } = {}) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Withdrawal Operations!A2:D", valueRenderOption: "UNFORMATTED_VALUE" });
  const entries = (res.data.values || []).filter((row) => row[0]).map(parseWithdrawalOperationRow);
  return verify ? assertOperationalLedgerEntries("withdrawal_operation", entries) : entries;
}
...
export const SYSLOOP_EXPECTED_HEADERS = {
  ...
  "Withdrawal Operations": { row: 1, headers: WITHDRAWAL_OPERATION_HEADERS },
