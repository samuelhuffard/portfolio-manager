...
import { investorLedgerRow, parseInvestorLedgerRow } from "./investor-ledger.js";
...
const TABS = ["Overview", "Holdings", "Performance", "Trade Ledger", "Lots", "Investors", "Track Record", ...AGENT_TAB_NAMES];
...
export async function readCashBalance(sheets, spreadsheetId) {
  ...
  const cashRow = (res.data.values || []).find((row) => row[0] === "Cash");
  return cashRow && cashRow[5] !== "" && cashRow[5] != null ? Number(cashRow[5]) : 0;
}
...
export async function readHoldingsAllocation(sheets, spreadsheetId) {
  ...
  return (res.data.values || [])
    .filter((row) => row[0] && row[0] !== "Cash" && !String(row[0]).startsWith("Last synced") && !String(row[0]).startsWith("⚠️"))
    .map((row) => ({ ticker: row[0], marketValue: row[5] !== "" && row[5] != null ? Number(row[5]) : null }));
}
...
export async function readHoldingsReturnPct(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Holdings!A2:I" });
  const out = {};
  for (const row of res.data.values || []) {
    const ticker = row[0];
    if (!ticker || ticker === "Cash" || String(ticker).startsWith("Last synced") || String(ticker).startsWith("⚠️")) continue;
    const raw = row[8];
...
const PERFORMANCE_HEADERS = ["Date", "Portfolio Value", "S&P 500 (SPY)", "Units Outstanding", "NAV per Unit"];

export async function appendPerformanceRow(sheets, spreadsheetId, sheetId, { date, portfolioValue, spyPrice, unitsOutstanding, navPerUnit }) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: { values: [[date, portfolioValue, spyPrice, unitsOutstanding ?? "", navPerUnit ?? ""]] },
  });
}
...
export async function readPerformanceHistory(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Performance!A2:E" });
...
export async function readInvestorLedger(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Investors!A2:J" });
  return (res.data.values || [])
    .filter((row) => row[0])
...
const TRADE_LEDGER_HEADERS = [
  "Date", "Ticker", "Side", "Shares", "Price", "Amount ($)", "Order ID", "Agent ID", "Proposal ID", "Realized Gain ($)",
];
...
export async function appendTradeLedgerEntries(sheets, spreadsheetId, sheetId, trades) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: trades.map((t) => [
        t.date, t.ticker, t.side, t.shares, t.price, t.amount, t.orderId ?? "",
        t.agentId ?? "unattributed", t.proposalId ?? "", t.realizedGain ?? "",
      ]),
    },
  });
}

export async function readTradeLedger(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Trade Ledger!A2:J" });
  return (res.data.values || []).filter((row) => row[0]).map((row) => ({
    date: row[0],
    ticker: row[1],
    side: row[2],
    shares: Number(row[3]),
...
const LOTS_HEADERS = ["Lot ID", "Ticker", "Open Date", "Agent ID", "Cost Per Share", "Shares Original", "Shares Open", "Status"];
...
export async function appendLots(sheets, spreadsheetId, sheetId, lots) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    insertDataOption: "INSERT_ROWS",
    requestBody: {
      values: lots.map((l) => [l.lotId, l.ticker, l.openDate, l.agentId, l.costPerShare, l.sharesOriginal, l.sharesOpen, l.status]),
    },
  });
}

/** Full lot list (all tickers, all statuses) — caller filters as needed (see lib/tax-lots.js). */
export async function readAllLots(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Lots!A2:H" });
  return (res.data.values || []).filter((row) => row[0]).map((row, i) => ({
...
export async function applyLotUpdatesToSheet(sheets, spreadsheetId, updatedLots) {
  ...
  const data = updatedLots.map((lot) => ({
    range: `Lots!G${lot.rowIndex + 2}:H${lot.rowIndex + 2}`,
    values: [[lot.sharesOpen, lot.status]],
  }));
  await sheets.spreadsheets.values.batchUpdate({ spreadsheetId, requestBody: { valueInputOption: "RAW", data } });
}
