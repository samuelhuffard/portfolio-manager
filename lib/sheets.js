...
import { assertOperationalLedgerEntries, signOperationalLedgerEntry } from "./operational-ledger.js";
...
const PERFORMANCE_HEADERS = ["Date", "Portfolio Value", "S&P 500 (SPY)", "Units Outstanding", "NAV per Unit", "Row HMAC"];
...
export async function appendPerformanceRow(sheets, spreadsheetId, sheetId, { date, portfolioValue, spyPrice, unitsOutstanding, navPerUnit }) {
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: { values: [[date, portfolioValue, spyPrice ?? "", unitsOutstanding ?? "", navPerUnit ?? "", entry.rowHmac]] },
  });
}
...
export async function readPerformanceHistory(sheets, spreadsheetId, { verify = true } = {}) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Performance!A2:F", valueRenderOption: "UNFORMATTED_VALUE" });
  const entries = (res.data.values || []).filter((row) => row[0]).map(parsePerformanceRow);
  return verify ? assertOperationalLedgerEntries("performance", entries) : entries;
}
