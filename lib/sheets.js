...
const TABS = ["Overview", "Holdings", "Performance", "Recommendations", "Strategy", "Track Record", "Investors"];
 ...
 * expected to check their own Redis cache (see getCachedSpreadsheetId) before
...
const INVESTOR_HEADERS = ["Date", "Investor Email", "Investor Name", "Type", "Amount ($)", "NAV per Unit", "Units"];

export async function appendInvestorLedgerEntry(sheets, spreadsheetId, sheetId, { date, email, name, type, amount, navPerUnit, units }) {
  ...
  const res = await sheets.spreadsheets.values.append({
    ...
    valueInputOption: "RAW",
    insertDataOption: "INSERT_ROWS",
    requestBody: { values: [[date, email, name, type, amount, navPerUnit, units]] },
  });

  const match = res.data.updates?.updatedRange?.match(/![A-Z]+(\d+):/);
  if (!match) return;
  const row0 = Number(match[1]) - 1;
  const bg = type === "Withdrawal" ? C_RED : C_GREEN;
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId,
...
export async function readInvestorLedger(sheets, spreadsheetId) {
  const res = await sheets.spreadsheets.values.get({ spreadsheetId, range: "Investors!A2:G" });
  return (res.data.values || [])
    .filter((row) => row[0])
    .map((row) => ({
      date: row[0],
...
const REC_NUMBER_FORMATS = {
  ...
  18: { type: "NUMBER", pattern: PLAIN_PERCENT_FMT },
  19: { type: "NUMBER", pattern: "0.00" },
};
