...
export function withdrawalPlanMatchesRequest(plan, { email, investorId, requestedAmount, sales }) {
  validateWithdrawalCommitPlan(plan);
  // Identity is (ticker, shares) only — price is stripped below and a stored
  // plan never retains one. Substitute a placeholder so this comparison cannot
  // throw on a caller that has no live price to hand: refusing to *compare* a
  // request would turn a recoverable retry into a hard stop for a field that
  // does not participate in the comparison.
  if (!Array.isArray(sales)) throw new TypeError("sell instructions must be an array.");
  const requestedSales = normalizedSales(sales.map((sale) => ({ ...sale, price: 1 })))
