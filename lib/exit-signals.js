...
export function evaluateExitSignals(input, opts = {}) {
  const { closes = [], benchmarkCloses = null, fundamental = null } = input;
  ...
  const dataUnavailable = priceDataMissing && !t3;

  return { t1, t2, t3, dataUnavailable, detail, reasons };
}
...
export function resolveExitAction(signals, opts = {}) {
  const { t1, t2, t3, dataUnavailable } = signals;
