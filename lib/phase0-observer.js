import { RESEARCH_FAILURE_OUTCOMES, RESEARCH_OUTCOME_KINDS } from "./research-run-report.js";
import { holdingMonitorCoveragePasses } from "./holding-monitor-coverage.js";
import { MCP_ACCOUNT_POLICY_VERSION, assertMcpReadReceipt } from "../contracts/mcp-read-job.js";

const FAILURE_OUTCOMES = RESEARCH_FAILURE_OUTCOMES;
...
function researchCheck(research) {
  ...
  if (reports.length === 0) {
    // requiredRunPresent=false means a same-day scheduled run was due and is
    // missing — that is a cadence failure, never "no sample due". Only a day
    // with no required run (Friday) and an empty unconsumed history may pass.
    if (research?.requiredRunPresent === false) {
      return check("research_accounting", "SKILL", "fail", "The required same-day scheduled research run is missing from history.", {
        fresh: research?.fresh ?? false, reason: "cadence_missing", runIds: [], validResearchSamples: 0,
      });
    }
    if (research?.requiredRunPresent === true) {
    ...
    return check("research_accounting", "SKILL", "insufficient", "Scheduled research history is unreadable or the run requirement is unknown.", {
      fresh: research?.fresh ?? false, reason: "history_unreadable", runIds: [], validResearchSamples: 0,
    });
  }
...
function validMcpReceipt(row, dateET, secret) {
  let receipt;
  try {
    receipt = assertMcpReadReceipt(row, secret);
  } catch {
    return false;
  }
...
function matchingOperatorException(exceptions, { job, invocationId = null } = {}) {
  return (exceptions ?? []).find((exception) => exception?.approvedBy === "sam"
    && exception?.kind === "operator_managed_claude_cli_capacity"
    && exception?.job === job
    && (invocationId == null || exception?.invocationId === invocationId)) ?? null;
}

function matchingSentinelException(anomaly, criticalJobs, operatorExceptions, dateET) {
  // The sentinel's cron fingerprint names a failed job but not its invocation.
  // Reconstruct that link only from the current critical-job run, which carries
  // the invocation id required by the exception. A missed or unrelated P1 is
  // therefore never waived.
  if (anomaly?.check !== "cron" || typeof anomaly?.fingerprint !== "string") return null;
  const match = anomaly.fingerprint.match(/^(.*):failed$/);
  if (!match) return null;
  const job = (criticalJobs ?? []).find((candidate) => candidate?.name === match[1]
    && candidate?.run?.dateET === dateET
    && candidate?.run?.ok === false);
  return job ? matchingOperatorException(operatorExceptions, { job: job.name, invocationId: job.run.invocationId }) : null;
}

function scheduledInvocationCheck(schedules, dateET, mcpReceiptSecret, operatorExceptions = [], criticalJobs = []) {
  ...
  for (const schedule of schedules) {
    ...
    for (const invocationId of expected) {
      ...
      const exception = matchingOperatorException(operatorExceptions, { job: schedule?.name, invocationId });
      if (matches.length === 0) {
        if (exception) { waived.push({ job: schedule?.name, invocationId, exception }); continue; }
        issues.push({ job: schedule?.name, invocationId, reason: "missing" });
        continue;
      }
      // Append-only history keeps failures visible, but an invocation may be
      // retried safely. Its final retained attempt is authoritative; a later
      // failure cannot be masked by an earlier success.
      const finalAttempt = matches.at(-1);
      if (finalAttempt?.ok !== true) {
        if (exception) { waived.push({ job: schedule?.name, invocationId, exception, finalAttempt }); continue; }
      ...
      if (["holdings-sync", "order-reconciliation"].includes(schedule?.name)
        && !validMcpReceipt(finalAttempt, dateET, mcpReceiptSecret)) {
        issues.push({ job: schedule.name, invocationId, reason: "invalid_account_bound_mcp_receipt" });
      }
      if (schedule?.name === "system-sentinel"
        && !Array.isArray(finalAttempt?.evidence?.blockingAnomalies)) {
        issues.push({ job: schedule.name, invocationId, reason: "blocking_or_unreadable_sentinel_result" });
      }
      if (schedule?.name === "system-sentinel" && Array.isArray(finalAttempt?.evidence?.blockingAnomalies)) {
        const active = finalAttempt.evidence.blockingAnomalies.filter((anomaly) => !matchingSentinelException(anomaly, criticalJobs, operatorExceptions, dateET));
        if (active.length > 0) issues.push({ job: schedule.name, invocationId, reason: "blocking_sentinel_result", active });
      }
      if (schedule?.name === "intraday-monitor"
        && !holdingMonitorCoveragePasses(finalAttempt?.evidence?.holdingMonitoring)) {
        issues.push({ job: schedule.name, invocationId, reason: "invalid_holding_coverage" });
      }
    }
  }
  return issues.length === 0
    ? check("scheduled_invocations", "TRUST", "pass", `All ${expectedCount} scheduled invocations have current-day non-masking evidence${waived.length ? `; ${waived.length} approved operator-capacity exception(s) remain visible.` : "."}`, { schedules, waived })
...
export function buildPhase0Observation(input) {
  ...
  const operatorExceptions = Array.isArray(input?.operatorExceptions) ? input.operatorExceptions : [];
  const policyRelease = input?.operatorPolicyRelease;
  ...
  const deploymentComplete = Boolean(
    ...
    && Number.isFinite(Date.parse(deployment?.startedAt ?? ""))
  ...
  checks.push(deploymentComplete
    ? check("deployment_identity", "TRUST", "pass", "Deployed commit, branch, start time, mandate versions, and research-selection policy are identified.", deployment)
    : check("deployment_identity", "TRUST", "insufficient", "Deployed commit/branch/start time or an active policy version is missing.", deployment ?? null));

  const deploymentDate = Number.isFinite(Date.parse(deployment?.startedAt ?? ""))
    ? new Intl.DateTimeFormat("en-CA", { timeZone: "America/New_York" }).format(new Date(deployment.startedAt))
    : null;
  const policyOnlyRelease = policyRelease?.dateET === dateET
    && policyRelease?.approvedBy === "sam"
    && policyRelease?.scope === "phase0_observer_only";
  checks.push((deploymentDate && deploymentDate < dateET) || policyOnlyRelease
    ? check("deployment_eligibility", "TRUST", "pass", policyOnlyRelease ? "A Sam-approved operator-capacity policy-only release permits this date to remain eligible." : `The gate-closing release predates ${dateET}; this trading day is eligible to count.`, { deploymentDate, observationDate: dateET, policyOnlyRelease })
    : check("deployment_eligibility", "TRUST", "fail", "A safety day can count only on the first trading day after the gate-closing release.", { deploymentDate, observationDate: dateET }));
  ...
  if (jobs.length === 0) {
    ...
    for (const domain of ["TRUST", "SKILL"]) {
      const scoped = jobs.filter((job) => (job?.domain ?? "TRUST") === domain);
      if (!scoped.length) continue;
      const bad = scoped.filter((job) => !job?.run || job.run.dateET !== dateET || (job.run.ok !== true && !matchingOperatorException(operatorExceptions, { job: job.name, invocationId: job.run.invocationId })) || job.run.skippedHoliday);
      const name = domain === "TRUST" ? "critical_jobs" : "skill_jobs";
      checks.push(bad.length === 0
        ? check(name, domain, "pass", `All ${scoped.length} due ${domain} job(s) recorded successful current-day runs.`, scoped)
        : check(name, domain, "fail", `${bad.length} due ${domain} job(s) are missing, stale, skipped, or failed.`, bad));
    }
  }

  checks.push(scheduledInvocationCheck(input?.scheduledInvocations, dateET, input?.mcpReceiptSecret, operatorExceptions, jobs));
  if (operatorExceptions.length) checks.push(check("operator_exceptions", "TRUST", "warning", `${operatorExceptions.length} Sam-approved operator-capacity exception(s) applied; underlying receipts remain retained.`, operatorExceptions));
  ...
  if (!monitoring || monitoring.length === 0) {
    ...
    const monitored = monitoring.reduce((sum, entry) => sum + (entry?.coverage?.monitored ?? 0), 0);
    checks.push(bad.length > 0
      ? check(
          "holding_monitoring",
          "TRUST",
      ...
      : expected > 0 && monitored === 0
        ? check(
          "holding_monitoring",
          "TRUST",
          "insufficient",
          `${monitoring.length} due monitor(s) accounted for ${expected} held-name checks, but every check used degraded evidence.`,
          monitoring
        )
        : check(
          "holding_monitoring",
  ...
  const p1 = input?.sentinel?.fresh === true && Array.isArray(input.sentinel.anomalies)
    ? input.sentinel.anomalies.filter((row) => (row?.severity === "P0" || row?.severity === "P1")
      && !matchingSentinelException(row, jobs, operatorExceptions, dateET))
    : null;
  checks.push(p1 == null
