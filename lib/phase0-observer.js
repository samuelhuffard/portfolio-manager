...
function scheduledInvocationCheck(schedules, dateET, mcpReceiptSecret, operatorExceptions = []) {
    ...
    for (const invocationId of expected) {
      ...
      if (schedule?.name === "system-sentinel"
        && (!Array.isArray(finalAttempt?.evidence?.blockingAnomalies)
          || finalAttempt.evidence.blockingAnomalies.length > 0)) {
...
export function buildPhase0Observation(input) {
  ...
  checks.push(scheduledInvocationCheck(input?.scheduledInvocations, dateET, input?.mcpReceiptSecret, operatorExceptions));
