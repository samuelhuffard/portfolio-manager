...
function scheduledInvocationCheck(schedules, dateET, mcpReceiptSecret) {
...
export function buildPhase0Observation(input) {
  ...
  const deploymentDate = Number.isFinite(Date.parse(deployment?.startedAt ?? ""))
    ...
    : null;
  checks.push(deploymentDate && deploymentDate < dateET
    ? check("deployment_eligibility", "TRUST", "pass", `The gate-closing release predates ${dateET}; this trading day is eligible to count.`, { deploymentDate, observationDate: dateET })
  ...
  if (jobs.length === 0) {
    ...
    for (const domain of ["TRUST", "SKILL"]) {
      ...
      const bad = scoped.filter((job) => !job?.run || job.run.dateET !== dateET || job.run.ok !== true || job.run.skippedHoliday);
  ...
  checks.push(scheduledInvocationCheck(input?.scheduledInvocations, dateET, input?.mcpReceiptSecret));
