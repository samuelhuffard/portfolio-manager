...
import { signOperationalLedgerEntry } from "./operational-ledger.js";
...
export async function recordReconciliationNeeded({ orderId, proposalId, ticker, side, shares, reason }) {
  ...
  const redis = getRedis();
  if (!redis) {
    // Money-adjacent OUTPUT path — must fail loudly, never silently drop.
    console.error(`[Redis] NOT CONFIGURED — reconciliation record DROPPED for order ${orderId} (${side} ${ticker}). This mismatch is now untracked.`);
    return null;
  }
...
export async function listOpenReconciliations() {
  const redis = getRedis();
  if (!redis) return [];
  const ids = await redis.smembers(RECON_OPEN_SET);
  if (!ids?.length) return [];
  const items = await Promise.all(ids.map((id) => redis.get(`${RECON_PREFIX}${id}`).catch(() => null)));
  return items.map((v) => (typeof v === "string" ? JSON.parse(v) : v)).filter(Boolean);
}
