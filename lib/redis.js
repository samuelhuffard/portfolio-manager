...
import { PROPOSAL_EXPIRY_MS, TICKER_RE } from "../contracts/proposal.js";
import { signOperationalLedgerEntry, computeOperationalLedgerHmac, getOperationalLedgerSecret } from "./operational-ledger.js";
...
const RESEARCH_DATA_STATUS_FIELDS = [
...
const SHADOW_SELECTION_STATUS_FIELDS = [
...
const SHADOW_SELECTION_REASON_CODES = new Set([
...
function normalizeShadowReasonCodeCounts(value) {
  ...
  return Object.fromEntries(Object.entries(value)
    .filter(([key, count]) => SHADOW_SELECTION_REASON_CODES.has(key) && Number.isInteger(count) && count >= 0)
...
export async function setResearchScanStatus(status) {
  const redis = getRedis();
  if (!redis) {
  ...
  try {
    ...
    if (payload.status === "completed" || payload.status === "failed") {
      await redis.lpush(RESEARCH_SCAN_HISTORY_KEY, JSON.stringify(payload));
      await redis.ltrim(RESEARCH_SCAN_HISTORY_KEY, 0, RESEARCH_SCAN_HISTORY_MAX - 1);
      await redis.expire(RESEARCH_SCAN_HISTORY_KEY, RESEARCH_SCAN_TTL);
    }
  } catch (e) {
    console.warn("[Redis] Failed to set research scan status:", e.message);
  }
}
...
export async function listOpenReconciliations() {
  ...
  return items.map((rec) => {
    const { rowHmac, ...fields } = rec;
    const verified = Boolean(secret) && Boolean(rowHmac) && computeOperationalLedgerHmac("reconciliation", fields, secret) === rowHmac;
    return { ...rec, verified };
  });
}

export async function clearReconciliation(orderId) {
  const redis = getRedis();
  if (!redis) return;
  await redis.srem(RECON_OPEN_SET, String(orderId));
  await redis.del(`${RECON_PREFIX}${orderId}`);
}
