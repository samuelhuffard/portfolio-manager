...
const HWM_EPSILON = 1e-9;

/**
 * Reconciles a freshly rebuilt signed-ledger NAV basis with the persisted
 * safety ratchet. Scheduled scans may raise the HWM or row-count watermark,
 * but they may not lower either one. A deliberate breaker:recompute --apply
 * replaces the persisted record after a human has investigated the ledger.
 */
export function reconcileNavHighWaterMark({ ledgerHighWaterMark, dailyRows, stored }) {
  const ledgerHwm = Number(ledgerHighWaterMark);
  const observedDailyRows = Number(dailyRows);
  const storedHwm = stored?.basis === "navPerUnit" ? Number(stored.value) : NaN;
  const storedDailyRows = stored?.basis === "navPerUnit" ? Number(stored.dailyRows) : NaN;

  const validLedgerHwm = Number.isFinite(ledgerHwm) && ledgerHwm > 0;
  const validStoredHwm = Number.isFinite(storedHwm) && storedHwm > 0;
  const validObservedRows = Number.isInteger(observedDailyRows) && observedDailyRows >= 0;
  const validStoredRows = Number.isInteger(storedDailyRows) && storedDailyRows >= 0;

  const highWaterMark = validStoredHwm && validLedgerHwm
    ? Math.max(storedHwm, ledgerHwm)
    : validStoredHwm ? storedHwm : validLedgerHwm ? ledgerHwm : null;
  const retainedDailyRows = validStoredRows && validObservedRows
    ? Math.max(storedDailyRows, observedDailyRows)
    : validStoredRows ? storedDailyRows : validObservedRows ? observedDailyRows : null;

  const issues = [];
  if (validStoredHwm && validLedgerHwm && ledgerHwm < storedHwm - HWM_EPSILON) {
    issues.push("ledger_high_water_mark_decreased");
  }
  if (validStoredRows && validObservedRows && observedDailyRows < storedDailyRows) {
    issues.push("signed_daily_row_count_decreased");
  }

  const integrityAlertKey = issues.length
    ? [issues.join("+"), storedHwm, ledgerHwm, storedDailyRows, observedDailyRows].join(":")
    : null;

  return {
    highWaterMark,
    dailyRows: retainedDailyRows,
    observedDailyRows: validObservedRows ? observedDailyRows : null,
    ledgerHighWaterMark: validLedgerHwm ? ledgerHwm : null,
    issues,
    integrityAlertKey,
    shouldAlert: Boolean(integrityAlertKey && integrityAlertKey !== stored?.lastIntegrityAlertKey),
  };
}
