...
async function requireRedis() {
  ...
  if (!redis) throw new Error("Redis is required for Kairos shadow state.");
  return redis;
}
...
async function readParsed(redis, key, schema) {
  ...
  if (!parsed.success) throw new Error(`Kairos shadow state at ${key} failed validation.`);
  return parsed.data;
}
...
async function writeImmutable(redis, key, value, schema) {
  ...
  if (!stored || stableJson(stored) !== stableJson(parsed)) {
    throw new Error(`Kairos immutable state conflict at ${key}.`);
  }
  return stored;
}
...
export async function persistPortfolioManagerShadowReview(rawRequest) {
  ...
  if (!activePolicy) throw new Error("Kairos shadow policy is not activated yet.");
  if (stableJson(activePolicy) !== stableJson(request.policy)) {
    throw new Error("Review policy does not match the active Kairos shadow policy.");
  }
...
export async function getPortfolioManagerShadowState(limit = 100) {
  ...
  for (const value of values) {
    ...
    else if (value != null) console.error("[Kairos] Invalid persisted shadow decision skipped.");
  }
