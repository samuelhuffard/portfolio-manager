...
export function extractUsage(usage = {}, { cacheWriteTtl = null } = {}) {
  const source = usage ?? {};
  // A response with no input/output token counts cannot be priced. Record it
  // explicitly as incomplete instead of silently pricing the call at $0; the
  // monthly budget settles such calls conservatively at their reservation.
  const usageComplete = source.input_tokens != null && source.output_tokens != null;
  const inputTokens = numberOrZero(source.input_tokens, "usage.input_tokens");
  const outputTokens = numberOrZero(source.output_tokens, "usage.output_tokens");
  const cacheCreationInputTokens = numberOrZero(source.cache_creation_input_tokens, "usage.cache_creation_input_tokens");
  const cacheReadInputTokens = numberOrZero(source.cache_read_input_tokens, "usage.cache_read_input_tokens");
  const cacheClasses = cacheCreationBreakdown(source, cacheWriteTtl);
