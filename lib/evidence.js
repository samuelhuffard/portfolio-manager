...
export function fenceUntrusted(label, text, token) {
  // Content may come from a prior model that has seen this run's token, so it can
  // try to forge an early close (or nest a new fence). A marker needs BOTH its
  // prefix and this run's token, so strip both — but iterate the two removals
  // TOGETHER to a fixed point. Either removal can splice its neighbours back into
  // the other's pattern ("<<<END-" + token + "UNTRUSTED-…" rebuilds the prefix;
  // head + token + tail rebuilds the token), so one pass of each is escapable
  // while the combined loop is not. Each iteration strictly shortens the text.
  const runToken = String(token ?? "");
  let safeText = String(text ?? "");
  let previous;
  do {
    previous = safeText;
    safeText = safeText.replace(/<<<(?:END-)?UNTRUSTED-/g, "");
    if (runToken) safeText = safeText.split(runToken).join("");
