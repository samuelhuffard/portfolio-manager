 ...
 *
 * Known limitation (accepted, needs a product decision before changing): two
 * distinct investors sharing one email address pool their units here, so one
 * could withdraw against the other's balance. Tightening this to require
 * investorId agreement when both sides have one would be safer, but it changes
 * the identity semantics of every historical row — do not change it silently.
 * Until then, always record entries with --investor-id and never reuse an email
 * across investors.
 */
