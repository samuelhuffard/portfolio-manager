...
export async function fetchFundamentals(ticker) {
  ...
  try {
    ...
    console.warn(`[Yahoo] fetchFundamentals(${symbol}) failed:`, err.message);
...
export async function fetchDailyBars(ticker, { period1, period2 = new Date(), interval = "1d" } = {}) {
  try {
    ...
    console.warn(`[Yahoo] fetchDailyBars(${ticker}) failed:`, err.message);
    return [];
  }
}
...
export async function fetchEarningsSurprise(ticker) {
  ...
  try {
    ...
    console.warn(`[Yahoo] fetchEarningsSurprise(${symbol}) failed:`, err.message);
    return null;
  }
}
