...
// Robinhood fractional shares can carry more precision than the four-decimal
// display format used by older lot records. Keep share quantities precise so a
// signed SELL ceiling can never be rounded above the broker-held position.
...
export function openLot({ ticker, shares, costPerShare, date, agentId, lotId }) {
  ...
  return {
    ...
    sharesOriginal: roundShares(shares),
    sharesOpen: roundShares(shares),
    status: "OPEN",
  };
}
...
export function consumeLotsFIFO(openLots, ticker, sharesToSell, sellPricePerShare) {
  ...
  for (const lot of candidates) {
    ...
    lotsConsumed.push({
      lotId: lot.lotId,
      ticker,
      sharesConsumed: roundShares(sharesFromLot),
    ...
    const sharesOpenAfter = roundShares(lot.sharesOpen - sharesFromLot);
    updatedLots.push({
      ...lot,
