...
export function auditProposalQualityShadow({ agentId, candidate = {}, proposal = {}, baseline = {}, enriched = {} } = {}) {
  ...
  // HOLD explanations can summarize evidence freely; only an actionable draft
  // needs the stricter rank-versus-raw metric gate.
  const rankMisuse = isActionable(action) ? detectRankAsRawMetric(proposal, evidenceById) : [];
  const blockers = [
