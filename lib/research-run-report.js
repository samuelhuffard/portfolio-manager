...
export const RESEARCH_OUTCOME_KINDS = Object.freeze([
  "investment_hold",
  // Additive and forward-only: only an explicit structured marker below can
  // create this outcome. Missing legacy facts retain their existing outcome.
  "generator_degraded",
  "data_gate",
  "stale_data",
  "budget_exhausted",
...
const GENERATOR_DEGRADATION_KINDS = new Set(["parse", "truncation", "schema", "missing_required_field"]);
...
function factsAreContradictory(facts) {
  ...
  if (generatorDegradation != null && !GENERATOR_DEGRADATION_KINDS.has(generatorDegradation)) return true;
  ...
  if (
    generatorDegradation != null &&
    (dataGateBlocked || dataGateStale || failureKind != null || generatorAction != null || finalAction != null || riskOverridden || evaluatorState !== "not_run" || duplicateOpen || proposalDisposition !== "not_applicable")
  ) return true;
...
export function classifyRecommendationOutcome(facts) {
  ...
  if (generatorDegradation != null) return "generator_degraded";
...
export function formatResearchRunReport(report) {
  ...
  const generatorDegraded = totals.outcomeCounts.generator_degraded ?? 0;
  const degradationText = generatorDegraded > 0 ? `; ${generatorDegraded} generator degraded` : "";
  return `Research run ${report.runId ?? "unknown"}: ${totals.attemptedReviews} attempted; ${totals.outcomeCounts.investment_hold} investment HOLD${degradationText}; ${totals.outcomeCounts.proposal_created} proposals created; ${totals.outcomeCounts.unknown} unknown.`;
}
