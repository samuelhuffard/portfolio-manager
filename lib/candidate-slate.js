...
function rankScreened(screened) {
  return [...screened].sort((a, b) => {
    ...
    const aMom = a.fiftyTwoWeekChangePct ?? -Infinity;
    const bMom = b.fiftyTwoWeekChangePct ?? -Infinity;
...
export function buildSlate({ screened = [], holdings = [], scanTickers = [], ledger = {}, config = {}, now = new Date() }) {
  ...
  const neverResearched = rankScreened(screened.filter((c) => !picked.has(c.ticker) && !ledger[c.ticker]));
