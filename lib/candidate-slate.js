...
export function buildSlate({ screened = [], holdings = [], scanTickers = [], ledger = {}, config = {}, now = new Date() }) {
  ...
  const add = (ticker, bucket, { exempt = false } = {}) => {
    const t = ticker.toUpperCase();
    if (picked.has(t) || (!exempt && slate.length >= slateSize)) return;
    picked.add(t);
    slate.push({ ticker: t, bucket });
  };

  // Holdings are a monitoring obligation, not replaceable discovery capacity.
  // They remain present even when their count exceeds the nominal slate size.
  for (const t of holdings) add(t, "holdings", { exempt: true });
