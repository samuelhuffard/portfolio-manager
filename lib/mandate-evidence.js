 ...
 * Agent 2's EDGAR revenue series can establish only its unambiguous recent YoY
 * count/continuity fields. Consensus-dependent beats, adjusted-EPS persistence,
 * and undefined persistence terms remain null, so the shared scorer records
 * partial coverage. Special sectors still require their approved economic adapters
 * and fail closed.
 */

import { assembleConsensusEvidence } from "./consensus-snapshot.js";
import { deriveAgent3History } from "./agent3-history.js";
import { deriveAgent2RevenueHistory } from "./agent2-history.js";
...
/** Not derivable from `derived` alone — each needs the bundle named in OPTIONAL_BUNDLE_METRICS. */
 ...
 * all nine v3 metric ids, so no metric is structurally unbindable any more — but every
 * one of them still yields null (rescaled out, never zero) on thin evidence.
 */
export const OPTIONAL_BUNDLE_METRICS = Object.freeze({
  consensus: Object.freeze(["revBeat", "estimateRevisions"]),
  thirteenF: Object.freeze(["instOwnershipDir", "thirteenF"]),
 ...
 * estimateRevisions, and the 13F ownership pair. Mutates the caller's accumulators.
 *
 * Both bundles are optional and independently so. Omitting one leaves exactly its own
 * metrics unbound, which is what keeps a name with no snapshot history — or no 13F
 * coverage — scoring precisely as it did before either source existed.
 */
function applySharedEvidence({ absoluteEvidence, metricVector, bound, consensus, thirteenF }) {
  ...
  // instOwnershipDir + thirteenF from lib/thirteen-f.js. Q-004 (revised 2026-08-02):
  // required for FULL coverage, but absence degrades rather than blocks — a ~45-day-
  // delayed dataset must never hold a veto over a candidate.
  if (thirteenF) {
    if (thirteenF.instOwnershipDir) {
      absoluteEvidence.instOwnershipDir = thirteenF.instOwnershipDir;
      metricVector.instOwnershipDir = finite(thirteenF.instOwnershipDir.ownershipChangePoints);
      bind("instOwnershipDir");
    }
    if (thirteenF.thirteenF) {
      absoluteEvidence.thirteenF = thirteenF.thirteenF;
      metricVector.thirteenF = finite(thirteenF.thirteenF.latestQuarterChangePct);
      bind("thirteenF");
    }
  }
}
...
export function assembleMandateInputs({
  ...
  if (agentId === "agent-3") {
    ...
    applySharedEvidence({ absoluteEvidence, metricVector: longHorizonMetricVector, bound, consensus, thirteenF });
    return {
      supported: true,
  ...
  // Agent 1 consumes acceleration directly. For Agent 2, supplement the current
  // scalar with only the short-horizon revenue facts that its persisted EDGAR series
  // can prove. The other named persistence inputs deliberately remain null; see
  // lib/agent2-history.js for the policy boundary.
  if (finite(d.revYoY) != null) {
    const revenueHistory = agentId === "agent-2" ? deriveAgent2RevenueHistory(d._revenueQuarterSeries) : null;
    absoluteEvidence.revGrowth = agentId === "agent-1"
      ...
      : {
          ...
          positiveQuartersInLatestFour: revenueHistory?.positiveQuartersInLatestFour ?? null,
          positiveMultiQuarterPersistence: null,
          nonDecelerating: revenueHistory?.nonDecelerating ?? (finite(d.revAccel) == null ? null : d.revAccel >= 0),
  ...
  applySharedEvidence({ absoluteEvidence, metricVector, bound, consensus, thirteenF });
