...
/**
 * A vendor's fiscal-period end and EDGAR's reported period end for the SAME quarter
 * can differ by a few days (52/53-week retail calendars, month-end conventions).
 * Quarters are ~91 days apart, so this window admits the calendar skew without ever
 * being able to reach the adjacent quarter.
 */
export const PERIOD_MATCH_TOLERANCE_DAYS = 20;

/**
 * Pair a reported EDGAR quarter with the consensus this system observed BEFORE that
 * quarter was filed — the point-in-time selection `revenueBeatPct` documents as the
 * caller's responsibility, done once here so no caller has to re-derive it.
 *
 * Two rules make the pairing leakage-free, and both are the whole point of the
 * function:
 *   1. `retrievedAt < filed` — a snapshot taken after the report already contains the
 *      answer. Differencing against it would manufacture a ~0% beat for every name and
 *      look like excellent coverage while measuring nothing.
 *   2. the LATEST qualifying snapshot wins — the consensus a beat is judged against is
 *      the one standing immediately before the print, not a stale one from months back.
 *
 * Walks quarters newest-first and returns the first quarter that has a qualifying
 * pre-report snapshot, so one uncovered recent quarter does not discard older usable
 * evidence. Returns nulls (never a guess) when nothing pairs.
 *
 * @param {{ history?: object[], revenueQuarters?: object[], asOf?: string|null }} input
 *   `history` oldest → newest observed snapshots; `revenueQuarters` EDGAR quarterly
 *   revenue rows `{ end, val, filed }` oldest → newest.
 */
export function selectRevenueBeatPair({ history = [], revenueQuarters = [], asOf = null } = {}) {
  const cutoff = asOf == null ? null : Date.parse(asOf);
  const toleranceMs = PERIOD_MATCH_TOLERANCE_DAYS * 86_400_000;
  const usableQuarters = revenueQuarters
    .filter((quarter) => finite(quarter?.val) != null && quarter?.end && quarter?.filed)
    .filter((quarter) => cutoff == null || Date.parse(quarter.filed) <= cutoff)
    .sort((a, b) => Date.parse(b.end) - Date.parse(a.end));

  for (const quarter of usableQuarters) {
    const periodEnd = Date.parse(quarter.end);
    const filed = Date.parse(quarter.filed);
    if (!Number.isFinite(periodEnd) || !Number.isFinite(filed)) continue;
    const candidates = history.filter((snapshot) => {
      const snapshotEnd = Date.parse(snapshot?.periodEndDate ?? "");
      const retrieved = Date.parse(snapshot?.retrievedAt ?? "");
      if (!Number.isFinite(snapshotEnd) || !Number.isFinite(retrieved)) return false;
      if (finite(snapshot?.revenueAvg) == null) return false;
      return Math.abs(snapshotEnd - periodEnd) <= toleranceMs && retrieved < filed;
    });
    if (!candidates.length) continue;
    const snapshot = candidates.reduce((latest, row) =>
      Date.parse(row.retrievedAt) > Date.parse(latest.retrievedAt) ? row : latest);
    return { snapshot, actualRevenue: quarter.val, periodEnd: quarter.end, filedAt: quarter.filed };
  }
  return { snapshot: null, actualRevenue: null, periodEnd: null, filedAt: null };
}
