 ...
 * quoteSummary module, via the EXISTING `fetchFundamentals` call — `earningsTrend` is
 * appended to `FUNDAMENTALS_MODULES` in lib/yahoo.js, and Yahoo's quoteSummary accepts
 * a comma-separated module list in ONE request, so this adds zero HTTP requests, zero
 * data cost, and zero LLM tokens. No paid API is introduced.
 *
