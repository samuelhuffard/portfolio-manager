...
const FIELDS = {
  ...
  lot_reconciliation: ["reconciliationId", "lotId", "ticker", "agentId", "originalSharesOpen", "brokerShares", "originalLotHmac", "attestedBy", "attestation", "status", "createdAt", "sheetWriteAttemptedAt", "appliedAt"],
  legacy_approval_attestation: ["proposalId", "ticker", "side", "attestedBy", "attestation", "createdAt"],
  // The plan is append-only and signed before a withdrawal changes any money
  // state. It is the recovery authority for a retry, not Redis or a freshly
  // recomputed FIFO allocation against already-mutated lots.
  withdrawal_operation: ["operationId", "createdAt", "planJson"],
};
 ...
 * after long-lived rows have been deliberately re-signed/migrated and retained
 * immutable records have aged out, without removing investor/audit keys needed
...
// The scheduled slot is signed beside (rather than inside) the immutable
// historical money-state payload, preserving verification of older rows.
export function computePerformanceSourceInvocationHmac(entry, secret = getOperationalLedgerSecret()) {
  if (!entry.sourceInvocationId) return null;
  return createHmac("sha256", secret)
    .update(stableJson({ kind: "performance-source-invocation", sourceInvocationId: entry.sourceInvocationId, rowHmac: entry.rowHmac ?? null }))
    .digest("hex");
}
...
export function assertPerformanceSourceInvocationEntries(entries, secret = getOperationalLedgerSecret()) {
  const candidates = verificationCandidates(secret);
  for (const entry of entries) {
    if (!entry.sourceInvocationId && !entry.sourceInvocationHmac) continue; // historical row
    const matches = candidates.some((candidate) =>
      hmacsEqual(computePerformanceSourceInvocationHmac(entry, candidate) ?? "", entry.sourceInvocationHmac ?? ""));
    if (!matches) throw new Error("performance source-invocation integrity check failed.");
  }
  return entries;
}
