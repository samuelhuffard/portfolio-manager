...
import { InvestorLedgerEntrySchema, PositionSchema } from "../../contracts/accounting.js";
...
export async function shadowWriteCapitalEntry(entry) {
  return runShadowOperation("capital_entry", async (pool) => {
    ...
    const normalized = String(entry?.type ?? "").trim().toLowerCase() === "correction"
...
export async function shadowWritePosition(position) {
  return runShadowOperation("position", async (pool) => {
    ...
    await pool.query(
      `INSERT INTO positions (ticker, name, shares, avg_cost, cost_basis, market_value, updated_at)
        ...
        await client.query(
          `INSERT INTO positions (ticker, name, shares, avg_cost, cost_basis, market_value, updated_at)
           ...
           ON CONFLICT (ticker) DO UPDATE SET
             ...
             cost_basis = EXCLUDED.cost_basis, market_value = EXCLUDED.market_value, updated_at = now()`,
          [p.ticker, p.name ?? null, p.shares, p.avgCost, p.costBasis, p.marketValue]
        );
      }
