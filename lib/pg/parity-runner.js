...
export async function gatherPostgresShadow() {
  ...
  // Neon can cold-start or transiently throttle a burst of new pool connections.
  // This is a safety diagnostic, not a latency-sensitive request: read serially
  // so one parity pass does not manufacture an all-domain divergence from its
  // own five-query connection burst.
  const proposalRows = await pgRows(`SELECT id, agent_id::text AS "agentId", ticker, side::text AS side,
              ...
              FROM proposals`);
  const positionRows = await pgRows(`SELECT ticker, name, shares, avg_cost AS "avgCost", cost_basis AS "costBasis", market_value AS "marketValue",
  ...
  const lotRows = await pgRows(`SELECT lot_id AS "lotId", ticker, COALESCE(owner_agent_id::text, 'unattributed') AS "agentId",
  ...
  const capitalRows = await pgRows(`SELECT c.entry_id AS "entryId", c.entry_date::text AS date, i.email, i.name, c.type,
  ...
  const accountingRows = await pgRows(`SELECT snapshot_date::text AS date, total_value AS "totalValue", cash,
