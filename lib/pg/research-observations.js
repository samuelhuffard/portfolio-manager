...
// Serialize every JSONB parameter explicitly so array-shaped evidence is never
// sent as an invalid JSONB literal.
...
/**
 * Read the newest durable score that was knowable at `asOf`.
 *
 * Proposal gating must never read a later research observation: that would let a
 * scheduled scan use evidence collected after its own decision time.  Redis's
 * latest-view cache is deliberately not usable here because it has no such
 * point-in-time query and is noncanonical.
 */
export async function readLatestMandateScoreObservationAtOrBefore({ agentId, ticker, asOf } = {}, options = {}) {
  const normalizedAgentId = requireText(agentId, "agentId");
  const normalizedTicker = requireText(ticker, "ticker").toUpperCase();
  ...
  const row = oneRow(await queryWith(options, `
    SELECT payload
    FROM mandate_score_observations
    WHERE agent_id = $1 AND ticker = $2 AND observed_at <= $3
    ORDER BY observed_at DESC, id DESC
    LIMIT 1
  `, [normalizedAgentId, normalizedTicker, normalizedAsOf]));
  if (!row) return null;
  const payload = typeof row.payload === "string" ? JSON.parse(row.payload) : row.payload;
  return MandateScoreObservationSchema.parse(JSON.parse(canonicalJson(payload)));
}
...
export async function writeResearchJobStart(run, options = {}) {
  ...
  `, [
    ...
    normalized.skippedCount, normalized.errorCount, jsonbParam(normalized.coverageSummary),
  ]);
...
export async function writeUniverseSnapshot(snapshot, options = {}) {
  ...
  `, [
    ...
    normalized.eligibleCount, jsonbParam(normalized.membership), normalized.contentHash,
  ]);
...
export async function writeEvidenceSnapshot(snapshot, options = {}) {
  ...
  const inserted = await queryWith(options, `
    ...
    RETURNING id
  `, [
    normalized.id, normalized.ticker, normalized.observedAt, jsonbParam(normalized.payload),
    jsonbParam(normalized.sourceSummary), jsonbParam(normalized.freshnessSummary), normalized.contentHash,
  ]);
  if (changedRows(inserted)) return { id: normalized.id, inserted: true, contentHash: normalized.contentHash };
  await assertExistingHash(options, {
...
function observationParams(observation, hash) {
  return [
    ...
    observation.eligible, jsonbParam(observation.eligibilityReasonCodes), observation.score,
    observation.uncappedScore, observation.rawPoints, observation.maxAvailablePoints,
    observation.complete, observation.actionable, jsonbParam(observation.coverageMask),
    jsonbParam(observation.missingMetrics), jsonbParam(observation.criticalMissingMetrics), observation.fallbackMethod,
    ...
    jsonbParam(observation.metrics), jsonbParam(observation), hash,
  ];
}
...
export async function finishResearchJobRun(runId, summary, options = {}) {
  ...
  `, [
    ...
    jsonbParam(normalized.coverageSummary), jsonbParam(normalized.detail),
  ]);
...
export async function failResearchJobRun(runId, errorSummary, options = {}) {
  ...
  const updated = await queryWith(options, `
    ...
    WHERE run_id = $1 AND status = 'running'
    RETURNING run_id
  `, [normalizedRunId, failedAt, jsonbParam(detail)]);
