...
async function writeOneEvent(client, event, options) {
  ...
  `, [event.id, key, event.previousObservationId, event.currentObservationId, event.ticker, event.agentId,
    event.primaryCause, event.allCauses, event.delta, event.material, event.materialityPolicyVersion,
    event.researchEligible, event.reasonCodes, event.changedMetrics, event.coverageChanged, event.peerSetChanged,
    event.versionChanged, event.createdAt, payload, hash]);
...
async function writeSelectionRunAndItems(client, run, items) {
  ...
  `, [run.id, run.sourceRunId, run.policyVersion, run.mode, run.candidateCount, run.selectedCount,
    run.displacedCount, run.createdAt, run.completedAt, runPayload, runHash]);
  ...
  for (const item of items) {
    ...
    `, [item.id, item.selectionRunId, item.ticker, item.agentId, item.selected, item.rank, item.bucket,
      item.budgetExempt, item.protectedReason, item.reasonCodes, item.triggeringObservationId, item.triggeringEventId,
      item.comparedTicker, item.displacedTicker, payload, hash]);
