...
export function evaluateDataGates(input, limits = {}, { now = new Date(), maxBarAgeDays = 4 } = {}) {
  ...
  const availableDataScore = uniqueRequired.length
    ? Math.max(0, Math.round(((uniqueRequired.length - uniqueMissing.length) / uniqueRequired.length) * 100))
    : 100;

  return {
    ok: missing.length === 0 && !stale && reasons.length === 0,
    // Missing and stale are both fail-closed, but they are different operational
    // failures. Keep this flag truthful so outcome accounting can distinguish a
    // provider/schema gap from an old price bar.
    stale,
