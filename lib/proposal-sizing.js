 ...
 * result is then capped by currently available idle cash after preserving any
 * configured minimum cash reserve.
 *
...
export function sizeProposalAmount({
  ...
  if (action === "BUY") {
    ...
    const reservePct = Math.max(0, Number(limits.minCashReservePct) || 0);
    const requiredReserveDollars = totalPortfolioValue * reservePct / 100;
    const deployableCash =
      cashAvailable == null
        ? null
        : Math.max(0, cashAvailable - requiredReserveDollars);
    if (deployableCash != null && deployableCash <= 0) return null;
    ...
    if (deployableCash != null && raw > deployableCash) {
      raw = deployableCash;
      cashClamped = true;
    }
  } else if (action === "SELL") {
