...
function evaluateHoldingTriggers(policy, values, blockers, due) {
  ...
  if (policy.agentId === "agent-1") {
    ...
    const tierDrop = triggerValue(values, "convictionTierDrop", blockers, "number");
    const trendBreak = below50 != null && rsWeeks != null
      && below50 >= exitPolicy.consecutiveClosesBelow50DayThreshold
      && rsWeeks >= exitPolicy.relativeStrengthDecliningWeeksThreshold;
    ...
    if ((revDecel != null && revDecel >= exitPolicy.revenueDecelerationQuartersThreshold)
      || (epsDecel != null && epsDecel >= exitPolicy.epsDecelerationQuartersThreshold)
      || guidanceReset === true) {
      result = upgradeExit(result, { action: "SELL_FULL", urgency: "elevated", reasonCodes: ["multi_quarter_fundamental_deterioration"] });
    }
    if (deadQuarters != null && deadQuarters >= exitPolicy.deadQuartersThreshold && catalyst === false) {
    ...
    if (businessScore != null && businessScore < exitPolicy.businessScoreFloor) {
      result = upgradeExit(result, { action: "REVIEW_REQUIRED", urgency: "elevated", reasonCodes: ["annual_or_event_reunderwrite_below_65"] });
    }
    if (currentWeight != null && currentWeight > exitPolicy.positionDriftCeilingPct) {
      result = upgradeExit(result, { action: "SELL_PARTIAL", urgency: "routine", reasonCodes: ["position_drift_above_25_pct"] });
    }
    if (valuationPercentile != null && valuationPercentile >= exitPolicy.valuationPercentileCeiling) {
