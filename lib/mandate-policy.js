...
function evaluateHoldingTriggers(policy, values, blockers, due) {
  ...
  if (policy.agentId === "agent-1") {
    ...
    if (tradingDays != null && progress === false) {
      if (tradingDays >= 40 && catalyst === false) result = upgradeExit(result, { action: "SELL_FULL", urgency: "routine", reasonCodes: ["dead_trade_40_day_exit"] });
      else if (tradingDays >= 30) result = upgradeExit(result, { action: "SELL_PARTIAL", urgency: "routine", reasonCodes: ["dead_trade_30_day_trim"] });
      else if (tradingDays >= 20) result = upgradeExit(result, { action: "REVIEW_REQUIRED", urgency: "routine", reasonCodes: ["dead_trade_20_day_review"] });
    }
