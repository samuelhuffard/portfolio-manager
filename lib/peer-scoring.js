...
import { pickFallbackMode, THIN_PEER_CONVICTION_CAP } from "./peer-resolve.js";
...
/** Mandate §5 routing. `peerCount` excludes the candidate itself. */
export function fallbackMethodForPeerCount(peerCount) {
  if (!Number.isInteger(peerCount) || peerCount < 0) return "unavailable";
  return pickFallbackMode(peerCount).mode;
}

/** Apply the §5 thin-peer cap only after all submetrics have been aggregated. */
export function applyThinPeerConvictionCap(total, { thinPeerSet, humanOverride = false } = {}) {
  if (!Number.isFinite(total)) return null;
  if (thinPeerSet && !humanOverride) return Math.min(total, THIN_PEER_CONVICTION_CAP);
  return total;
}

/**
 * Score a scalar against deterministic absolute bands.
 * Bands are evaluated in order (best first) and use an inclusive lower or upper
 * boundary. A missing value is intentionally unscored rather than treated as zero.
 */
export function absoluteBandFraction(value, bands, higherIsBetter = true) {
  if (value == null || Number.isNaN(value) || !Array.isArray(bands)) return null;
  for (const band of bands) {
    const threshold = band.threshold;
    if (higherIsBetter ? value >= threshold : value <= threshold) return band.fraction;
  }
  return 0;
}

/** Score a metric using its absolute scalar table. */
export function scoreMetricAbsolute(value, spec, absoluteSpec) {
  const maxPoints = spec.points;
  const fraction = absoluteBandFraction(value, absoluteSpec?.bands, absoluteSpec?.higherIsBetter !== false);
  if (fraction == null) return { points: 0, maxPoints, fraction: null, missing: true, method: "absolute" };
  return { points: fraction * maxPoints, maxPoints, fraction, missing: false, method: "absolute" };
}

/**
 * Score a valuation using the mandated cascade: company history, broad-sector
 * distribution, then universal/special-sector absolute bands. Missing upstream
 * sources fall through; missing all sources remains unscored (never fabricated).
 */
export function scoreValuationCascade({
  value,
  companyHistory = [],
  companyHistoryCoverage = {},
  sectorValues = [],
  absoluteSpec,
  absoluteSource = "universal_absolute",
}) {
  if (value == null || Number.isNaN(value)) return { fraction: null, source: "missing", missing: true };
  const validHistory = companyHistory.filter((v) => v != null && !Number.isNaN(v));
  const historyCoverageOk = companyHistoryCoverage.years >= 3
    && (companyHistoryCoverage.quarterlyObservations >= 12 || companyHistoryCoverage.monthlyObservations >= 36);
  const requiredHistoryObservations = companyHistoryCoverage.quarterlyObservations >= 12 ? 12 : 36;
  if (historyCoverageOk && validHistory.length >= requiredHistoryObservations) {
    const percentile = percentileRank(value, [...validHistory, value], false);
    const fraction = percentile >= 0.67 ? 1 : percentile >= 0.5 ? 0.75 : percentile >= 0.33 ? 0.5 : 0;
    return { fraction, source: "company_history", missing: false };
  }
  const validSector = sectorValues.filter((v) => v != null && !Number.isNaN(v));
  if (validSector.length >= 8) {
    const percentile = percentileRank(value, [...validSector, value], false);
    const fraction = percentile >= 0.75 ? 1 : percentile >= 0.5 ? 0.75 : percentile >= 0.25 ? 0.5 : 0;
    return { fraction, source: "sector_benchmark", missing: false };
  }
  const fraction = absoluteBandFraction(value, absoluteSpec?.bands, absoluteSpec?.higherIsBetter !== false);
  return fraction == null
    ? { fraction: null, source: "missing", missing: true }
    : { fraction, source: absoluteSource, missing: false };
}
...
/**
 * Per-submetric §5 scorer. The absolute table is supplied by the mandate config;
 * this function is deliberately pure and remains outside the live scan path.
 */
export function scoreMetricWithFallback(value, peerValues, spec, absoluteSpec) {
  // PeerSet resolution returns true peers *without* the candidate. Keep this API
  // aligned so its count and percentile cannot accidentally include self twice.
  const peerCount = peerValues.filter((v) => v != null && !Number.isNaN(v)).length;
  const method = fallbackMethodForPeerCount(Math.max(0, peerCount));
  const absolute = scoreMetricAbsolute(value, spec, absoluteSpec);

  if (absolute.missing) return { ...absolute, peerCount, thinPeerSet: method !== "peer_relative" };
  if (method === "absolute") return { ...absolute, peerCount, thinPeerSet: true };

  const percentile = percentileRank(value, [...peerValues, value], spec.higherIsBetter !== false);
  const peerFraction = percentile == null ? null : bandFraction(percentile);
  const peerPoints = peerFraction == null ? null : peerFraction * spec.points;
  if (method === "blended_50_50") {
    return {
      points: (peerPoints == null ? 0 : peerPoints * 0.5) + absolute.points * 0.5,
      ...
      method,
      peerPoints,
      absolutePoints: absolute.points,
      missing: false,
    };
  }
  return {
